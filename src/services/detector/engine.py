    ...
    def analyze(
        ...
        try:
            ...
            if drift_seconds > DetectionEngine.MAX_CLOCK_DRIFT_SECONDS:
                return (
                    DataQualityStatus.ANOMALY_NOISE,
                    f"Clock Drift Anomaly: {drift_seconds}s delay detected.",
                )
        except TypeError:
        ...
        if len(baseline_values) < 5:
            return (
                ...
                "Insufficient historical baseline for statistical variance.",
            )
        ...
        if z_score > DetectionEngine.Z_SCORE_THRESHOLD:
            return (
                DataQualityStatus.ANOMALY_NOISE,
                f"Statistical Outlier: Z-Score {z_score:.2f} > {DetectionEngine.Z_SCORE_THRESHOLD}.",
            )
